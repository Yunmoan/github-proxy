...
const handleError=(error,req,res)=>{
  ...
  // 检查是否是CSP相关错误
  if(safeError.message&&safeError.message.includes('Content Security Policy')||safeError.message&&safeError.message.includes('CSP')){
    // 对CSP错误，返回更友好的响应
  ...
  // 检查是否涉及CDN域名
  if(req.url.includes('cdn.')&&req.url.includes('/fragment/')){
    // 尝试通过代理路径重新请求
  ...
  }else if(error.code==='ECONNABORTED'||safeError.message&&safeError.message.includes('timeout')){
  ...
  }else if(safeError.message&&safeError.message.includes('circular')||safeError.message&&safeError.message.includes('cyclic')){
    // 特别处理循环引用错误
